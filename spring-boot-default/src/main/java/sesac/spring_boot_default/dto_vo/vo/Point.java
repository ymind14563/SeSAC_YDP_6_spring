package sesac.spring_boot_default.dto_vo.vo;
...
public final class Point {
    ...
    // 두 점 사이의 거리 계산 (비지니스 로직)
    public double distanceTo(Point other) {
        int dx = this.x - other.x;
        int dy = this.y - other.y;
        return Math.sqrt(dx * dx + dy * dy); // 제곱근 함수
    }

    // equals(): 객체의 동등성 정의 (두 객체가 논리적으로 같은지)
    @Override
    public boolean equals(Object o) {
        // 현재 객치(this) 와 비교대상객체(매개변수 o) 가 같은 참조를 가르킨다면 (== 비교하므로) true
        if (this == o) return true;

        // 비교대상객체(매개변수 o) 가 null 이거나 현재 객체와 다른 클래스의 인스턴스라면 false
        if (o == null || getClass() != o.getClass()) return false;

        // 비교대상객체를 Point 타입으로 "형변환"
        Point point = (Point) o;
    ...
    @Override
    public int hashCode() {
        // Object 클래스의 hash 메서드 사용해서 x, y 값을 기반으로 해시 코드 생성
